...
marker just closes off the final segment.

So **4 markers → 3 exported clips:**
...
## Install (the easy way — with Claude Code)
...
[Claude Code](https://claude.com/claude-code) do it:
...
2. Open the folder in Claude Code and say:
   > **Install this Premiere panel for my OS.**

Claude Code will detect macOS vs Windows, enable unsigned CEP extensions, and
install the panel into the right folder. Then fully quit and reopen Premiere
Pro and find it under **Window → Extensions → Marker Export**.

## Install (manual)

If you'd rather not use Claude Code, run the installer for your platform from
the repo root:
...
5. **Export Markers** — clips are queued and run in Adobe Media Encoder.
